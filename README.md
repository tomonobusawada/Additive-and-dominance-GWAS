...
4. Association test\
   The simulated phenotypes are jointly regressed on the additive and dominance genotypes. We computed marginal association statistics using OLS: $\frac{1}{n}X_{A,j}^{T}\mathbf{y}$ and $\frac{1}{n}X_{D,j}^{T}\mathbf{y}$ for each SNP $j$.
